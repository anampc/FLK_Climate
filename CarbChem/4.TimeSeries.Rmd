...
```{r}
# Get the WS data
dm<-WS.data %>% select(SiteID, Date, Zone, Sub_region, Aragonite_Sat_W) 
...
regT<-regul(x=x$Y_M, y=y, n=27, units = "year", deltat=1/6)
plot(regT)
str(regT)

Mean_reg_WS<-extract(regT, n=27)
head(Mean_reg_WS)
plot(Mean_reg_WS[,1:7])
plot(Mean_reg_WS[,8:15])
...
Biscayne_Inshore<-(stl (Mean_reg_WS[,1], "per"))
acf(Biscayne_Inshore$time.series[,"remainder"])
plot(Biscayne_Inshore)

UK_Inshore<-(stl (Mean_reg_WS[,2], "per"))
...
<-(stl(log(co2), s.window = "per", t.window = 1000))
...
<-(stmd, set.pars = NULL, labels  =  NULL,
