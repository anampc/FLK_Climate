...
```{r, include=FALSE}
# 1. Get data
CC.data<-read.csv("Data/NCRMP_CarbonateChemistryData_3Files.csv", header = T)
  summary(CC.data)
  str((CC.data))
...
Notes:
* Regions are: DRTO, FGB, FLK, PR, SEFCRI, STX, USVI (STJ + STT)
* Locations are: DRTO  FGB   FL  FLK   PR  STJ  STT  STX
* How can I get lower, middle and upper keys?
* Site IDs are still showing duplicates (i.e., lowercase vs capital letters, spaces, etc) -> Fixed what I could
* Replace all N/As by NAs. It gives scary warnings of NAs introduced by coercion. What else is wrong? - > There was one space, verything else was miising data
* Changed all -9 in Arg, pH and pCO2 (4 samples, 12 values) to NA

* Row 131: DIC=4574.590
...
leaflet(Locations, height = '400px') %>% 
  ...
  fitBounds(lng1=-93.83, lat=28, lng2=-64.1, lat2=17.5)
...
* STT abd STJ locations overlap 


## Locations (Avergage geolocation for permanent samples)
...
kable(as.data.frame(GPS.Sites, format = "markdown"))
```


```{r makeAmap2, echo=FALSE}
leaflet(GPS.Sites, height = '400px') %>% 
  addTiles() %>%
  addProviderTiles(providers$Esri.WorldImagery)%>%
  #addProviderTiles(providers$CartoDB.Positron) %>% 
  #addMarkers(icon=coral.icon) %>% 
...
Map including the numer of samples collected by SiteID
...
### Only WS, year, Month 
```{r, echo=FALSE}
WS<-filter(CC.data, CC.data$Mission=="WaltonSmith")
WS$Month<-month(WS$Date)

Cases_Mission<-WS %>% count(
                                 Year,
                                 Month, sort = F)
...
kable(as.data.frame(Cases_Mission, format = "html"), caption="WS samples collected by year-month")
...
* Fixed some of these. Need to know which ones are unique / repeated
* ADCP_N is right?
* ADCP_SW is right?
...
```{r, echo=FALSE}
Sal_Temp<- ggplot(CC.data) + theme_bw() +
...
geom_point(aes (Salinity_Bottle, Temperature_C, fill=Season), alpha=0.5, shape=21)+
#geom_line(aes (Date, Temperature_C, colour=SiteID))+
...
theme(legend.position="top",
  ...
  geom_abline(slope = 0, intercept = 29.5, color="gray", linetype=2)+
  geom_vline(xintercept = 35, color="gray", linetype=2)
Sal_Temp
Sal_Temp+facet_wrap(~Region, scales="free")
