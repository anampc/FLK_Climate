...
# 2. Format variables
  ...
  CRM<-subset(CC.data,CC.data$CTDID=="CalVal_Jul2019_5" |
              CC.data$CTDID=="CalVal_Jul2019_6" |
                CC.data$CTDID=="CalVal_Jul2019_7" |
                CC.data$CTDID=="CalVal_Jul2019_8" |
                CC.data$CTDID=="CalVal_Jul2019_9" |
                CC.data$CTDID=="CalVal_Jul2019_10" |
                CC.data$CTDID=="CalVal_Jul2019_11" |
                CC.data$CTDID=="CalVal_Jul2019_12")
  
  CC.data<-CC.data[!CC.data$CTDID %in% CRM$CTDID, , drop = FALSE]
  
  
  # Remove CalVal
  CC.data<-filter(CC.data, Mission!="CalVal")
  ...
  #CC.data$DIC[CC.data$DIC == 4574.590 ] <- 2043.1832 (Done in data file)
  #CC.data$Salinity_Bottle[CC.data$Salinity_Bottle == 26.95700 ] <- 36.95700 (Done in file)
...
nTA_Sal<- ggplot(subset(CC.data, Region=="FLK")) + theme_bw() +
...
```{r, fig.height=8.8, fig.width=8.8}
nDIC_Sal<- ggplot(subset(CC.data, Region=="FLK")) + theme_bw() +
...
```{r}
library(ggpubr)
FLK_parameters<-ggarrange(nDIC_Sal, nTA_Sal,
...
```{r}
TA_DIC<- ggplot(subset(CC.data, Region=="FLK"), aes (nDIC, nTA)) + theme_bw() +
