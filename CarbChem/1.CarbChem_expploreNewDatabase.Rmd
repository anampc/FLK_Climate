...
```{r, include=FALSE}
# 1. Get data
CC.data<-read.csv("Data/ws-mysql-v2.csv", header = T)
  #summary(CC.data)
  str((CC.data))
...
* Add reef subregion: "Biscayne", "UK", "MK", "LK"
...
```{r}
Remove<-rbind(Salinity_Check, Omega_Check, TA_Check)
Remove <-Remove %>% 
        filter(!duplicated(CTDID))

write.csv(Remove, "FLK_results/ValuesToRemove.csv", row.names = F)

FLK.data_filtered<-FLK.data[!(FLK.data$CTDID %in% Remove$CTDID),]
write.csv(FLK.data_filtered, "FLK_results/FLK_filtered.csv", row.names = F)
```
