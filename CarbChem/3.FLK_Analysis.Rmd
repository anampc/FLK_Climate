...
leaflet(FLK.data[FLK.data$Year==2015,], height = '400px') %>% 
  addTiles() %>%
...
```{r, echo=FALSE}
Cases_Loc_Depht<-WS.data %>% count(Location,
                                  Zone,
                                  sort = F)
...
Cases_Loc_Depht<-as.data.frame(Cases_Loc_Depht %>%
                   ...
                   values_from = n))
write.csv(Cases_Loc_Depht, "Heidi.csv")
...
```{r}
All_ToD2<- ggplot(WS.data, aes(ToD, Aragonite_Sat_W)) + 
...
Arg_Temp<- ggplot(FLK.data) + theme_bw() +
...
Temp_months_all
Temp_months_all + facet_grid(~Zone) 
Temp_months_all + facet_grid(Zone~Sub_region)
  
Temp_months_all + facet_grid(Reference~Zone)
...
Sal_months_all
Sal_months_all + facet_grid(~Zone)  
Sal_months_all + facet_grid(Zone~Sub_region)
...
Aragonite_mon_year+facet_grid(Reference~Zone)
```
...
geom_smooth(method = "lm", aes(colour=Zone))+  
geom_point(aes(fill=factor(Zone)), shape=21, alpha=0.5, size=1)+
geom_abline(slope = 0, intercept = 3, color="gray", linetype=2)+
...
pCO2_all  
#ggsave(file="FLK_results/pCO2_a.pdf", plot=pCO2_all, dpi = 300, width=6, height=4)

#pCO2_all + facet_grid(~Zone)
pCO2_all_b<-pCO2_all + facet_grid(~Sub_region)
#ggsave(file="FLK_results/pCO2_b.pdf", plot=pCO2_all_b, dpi = 300, width=6, height=4)

pCO2_all_black<- ggplot(WS.data [WS.data$Extreme=="Normal", ],
                          aes(x=Date, y=pCO2)) +
...
pCO2_all_C<-pCO2_all_black + facet_grid(Zone~Sub_region)

#ggsave(file="FLK_results/pCO2_d.pdf", plot=pCO2_all_C, dpi = 300, width=6, height=6)
