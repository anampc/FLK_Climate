...
```{r}
TA_DIC<- ggplot(FLK.data, aes (nDIC, nTA)) + 
...
TA_DIC +
  facet_grid(Zone~Mission)

TA_DIC +
  facet_grid(Zone~Sub_region)

TA_DIC +
  facet_grid(Sub_region~Mission)
```
...
# Offsshore + Oceanic vs Inshore + Middle channel
...
```{r}
Aragonite_Reference<- ggplot(FLK.data,
...
Aragonite_Reference + facet_grid(Month~Mission)
Aragonite_Reference + facet_grid(Season~~Mission)
Aragonite_Reference + facet_grid(Precipitation~Mission)
```
