...
# Option 1
  ...
  NewData <- expand.grid(Month=unique(WS.open$Month),
                          Year=unique(WS.open$Year),
                          Sub_region=unique(WS.open$Sub_region),
                          Zone=unique(WS.open$Zone))
  pred_2 <- predict(Reference_LM , newdata=NewData, re.form= ~(1|Month),
                    level=0)
  #summary(pred_2) 
  #length(pred_2)
  head(pred_2)
  
  pred_2<-cbind(NewData, pred_2)
  
  
  Aragonite_Reference<- ggplot(pred_2,
                          aes(x=Year, y=pred_2)) +
...
**In shore Aragonite** = -0.074  Y +  152.498
**Mid Cahnner Aragonite** = -0.035  Y +  73.542  
