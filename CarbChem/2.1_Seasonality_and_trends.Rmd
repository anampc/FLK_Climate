...
**Figure 3:** Seasonal variation in Total Alkalinity (TA), Dissolved Inorganic Carbon (DIC) and aragonite satoration state (Omega) by region (colors) and shelf position (vertical panels).
...
# write.csv(Seasonal_Stats2, "FLK_results/Table_1_Season.csv", row.names = F)
...
Year_Add<- ddply (dMonth_Stats, .(Zone, Sub_region),
           summarise,
  ...
  scale_x_continuous(#limits = c(-350,340),
    ...
    scale_y_continuous(#limits = c(-350,340),
                     ...
                     name=expression(paste(Sigma, Delta, " TA (",
                             mu, "mol ", kg^-1, Year^-1, ")")))+ 
  geom_point(aes(dDIC_Y, dTA_Y, fill=Sub_region, shape=Zone) )+
...
#write.csv(dSeason_Stats, "FLK_results/Table1b.csv", row.names = F)
```
...
# write.csv(dSeasonal_Stats2, "FLK_results/Table_S2b_dSeason.csv", row.names = F)

```

**Note:** SD is NA when calculating it based on monthly meand (n=3 per season)

## - dYearly stats (monthly mean)
