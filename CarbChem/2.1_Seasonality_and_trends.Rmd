...
**Figure Supp 2:** Linear regressions of Friss normalized total alkalinity (nTA, µmol kg−1) as a function of measured Total Alcalinity (TA, µmol kg−1)
...
**Figure Supp 3:** Linear regressions of Friss normalized Dissolved Inorganc Carbon (nDIC, µmol kg−1) as a function of measured Dissolved Inorganc Carbon (DIC, µmol kg−1)
...
**Figure Supp 4:** Linear regressions of mean salinity normalized total alkalinity (n35TA, µmol kg−1) as a function of measured Total Alcalinity (TA, µmol kg−1)
...
**Figure Supp 5:** Linear regressions of mean salinity normalized Dissolved Inorganc Carbon (n35DIC, µmol kg−1) as a function of measured Dissolved Inorganc Carbon (DIC, µmol kg−1)
...
**Figure Supp 6:** Linear regressions of (A) TA-n35TA and (B) DIC-nDIC as a function of Salinity
...
TA_delta_Y<- DeltaYear_plot+
    ...
    geom_point(data=dMonth_Stats, aes (x=Sub_region, y=dTA_mean, fill=Season), 
...
dDIC_freq_pre<- Frequency_plot+
  geom_density(aes (dDIC_MY, fill = Precipitation), alpha = 0.4) +
  scale_x_continuous(breaks = seq(-300, 300, 100),
                    expression(paste(Delta, "DIC (", mu, "mol ", kg^-1, ")")))
dDIC_freq_pre
...
Omega_mc<-lmer(Aragonite_Sat_W ~ Year + Sub_region + (1 | Month), data = Trends.midchanel)
summary(Omega_mc) 
plot(Omega_mc)
...
Omega_of<-lmer(Aragonite_Sat_W ~ Year + Sub_region + (1 | Month), data =Trends.offshore)
plot(Omega_of)
...
pCO2_all<- Long_plot+
    ...
    geom_point(aes (x=Date, y=pCO2_uatm, fill=Precipitation), 
             alpha=0.8, size=1, shape=21)+
  geom_smooth(aes (x=Date, y=pCO2_uatm), method = "lm")
pCO2_all
pCO2_all+facet_grid(~Zone)
pCO2_all+facet_grid(~Zone) +
  geom_smooth(aes (x=Date, y=pCO2_uatm, colour=Sub_region), method = "lm")
...
pH_all+facet_grid(~Zone) +
  geom_smooth(aes (x=Date, y=pH_calculated, colour=Sub_region), method = "lm")
...
TA_all+facet_grid(~Zone) +
  geom_smooth(aes (x=Date, y=TA_umol_kg, colour=Sub_region), method = "lm")
...
TA_oc<-lmer(TA_umol_kg ~ (1 | Month), 
  ...
  summary(TA_oc)    
  plot(TA_oc)
  acf(resid(TA_oc), main="acf(resid(TA_ open))")
```
...
DIC_oc<-lmer(DIC_umol_kg ~ Year+ (1 | Month), data = Trends.ocean)
  summary(DIC_oc)    
  plot(DIC_oc)
  acf(resid(DIC_oc), main="acf(resid(DIC_ open))")
```
...
dTA_all+facet_grid(~Zone) +
  geom_smooth(aes (x=Date, y=dTA_MY, colour=Sub_region), method = "lm")
...
dTA_mc<-lmer(dTA_MY ~ SiteID + (1 | Month), 
             data = Trends.midchanel)
  summary(dTA_mc) 
  plot(dTA_mc)
...
dDIC_all<- Long_plot2+
    ...
    geom_point(aes (x=Date, y=dDIC_MY, fill=Precipitation), 
             alpha=0.8, size=1, shape=21)+
    geom_smooth(aes (x=Date, y=dDIC_MY), method = "lm")

dDIC_all
dDIC_all+facet_grid(~Zone)
dDIC_all+facet_grid(~Zone) +
  geom_smooth(aes (x=Date, y=dDIC_MY, colour=Sub_region), method = "lm")
