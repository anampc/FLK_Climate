...
pH_Om_Plot<-ggplot(data=dYearly_Stats, aes(x=dAra_M, y=dpH_M))+
  ...
  geom_linerange(aes(
    xmax = dAra_M + dAra_sd, xmin = dAra_M - dAra_sd, colour=factor(Sub_region)))+
  geom_linerange(aes(
    ymax = dpH_M + dpH_sd, ymin = dpH_M - dpH_sd, 
    colour=factor(Sub_region)))+
...
Trends_H<-ggarrange(Trends_a+ theme(axis.title.x=element_blank(),
                  ...
                  Trends_c + theme(legend.position = "none",
                                 strip.text.x = element_blank()),
          labels = c("A", "B", "C"),
          ncol = 1, nrow = 3, heights = c(1, 0.8, 0.9))
#Trends_H
...
TrendsV<-ggarrange(Trends_a+ 
          ...
          ncol = 3, nrow = 1)
TrendsV
#ggsave(file="FLK_results/Fig_2_Trends.svg", plot=TrendsV, dpi = 300, width=7.5, height=7)
```
