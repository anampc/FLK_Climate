...
leaflet(WS.GPS.Sites, height = '800px') %>%
  addTiles() %>%
  #addProviderTiles(providers$Esri.WorldImagery)%>%
  #addProviderTiles(providers$OpenStreetMap) %>%
...
**Figure 4: Inshore-ocean gradients for TA and DIC in the Florida Keys. (A) Seasonal ΔTA. (B) Seasonal ΔDIC.** Bars in A-B represent the mean Δ values per season, region, and shelf position (± 95 CI). (C) Scatter plot of yearly accumulated  ΣΔTA versus ΣΔDIC. The colors of the point represent the biogeographical region and the shapes the shelf zone.
...
p3a <- ggplot(Trends.inshore, aes(
  y=Aragonite_Sat_W, x=Year, colour = Sub_region)) +
  geom_point(alpha=0.8, size=0.8, shape=21) +
  geom_line(aes(y = predict(Omega_in)),size=1) 
print(p3a)
...
# ANOVAs
    ...
    # Putting all into one dataframe/table
    anova_Omega_In <- data.frame(cbind
    ...
    colnames(anova_Omega_In) <- c("", "Sum Sq", "Mean Sq", "N Df" , "D Df",
                                     "F value", "Pr(>F)")
    row.names(anova_Omega_In) <- NULL
    
    # create HTML table using kableExtra
    anova_Omega_In %>% kable("html", digits=2) %>% 
      kable_styling(bootstrap_options = "striped", full_width = F) %>% 
...
# R-ANOVAs    
    ...
    # Putting all into one dataframe/table
    ranova_Omega_In <- data.frame(cbind
    ...
    colnames(ranova_Omega_In) <- c("", "npar", "logLik", "AIC" , "LRT",
                                     "DF", "Pr(>Chi Sq)")
    row.names(ranova_Omega_In) <- NULL
    
    # create HTML table using kableExtra
    ranova_Omega_In %>% kable("html", digits=2) %>% 
      kable_styling(bootstrap_options = "striped", full_width = F) %>% 
      pack_rows(., "Omega inshore", 1, 3) %>% # groups rows with label
      pack_rows(., "Omega mid-channel", 4, 6)  %>% # groups rows with label
      pack_rows(., "Omega offshore", 7, 9) %>% # groups rows with label
...
modelsummary(Omega_models_0, stars = TRUE, 
             #statistic = c('std.error', 'p.value', 'conf.int'),
             title = 'Omega model outputs (all factors)',
             coef_map=factor_order)
#modelsummary(Omega_models, estimate = "p.value")
```
...
modelsummary(Omega_models, stars = TRUE, 
             #statistic = c('std.error', 'p.value', 'conf.int'),
             title = 'Omega model outputs (dropped factors)',
             coef_map=factor_order)
#modelsummary(Omega_models, estimate = "p.value")
```
